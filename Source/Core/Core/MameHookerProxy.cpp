...
void MameHookerProxy::Init()
{
  ...
  NOTICE_LOG_FMT(ACHIEVEMENTS, "INIT MameHookerProxy");
}
...
void MameHookerProxy::CloseGame()
{
  ...
  NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHooker : Close Game");
  if (processInfo != nullptr)
  {
    ...
    if (needForceClose)
    {
      NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHookerProxy : Force close");
      DWORD exitCode;
      if (GetExitCodeProcess((*processInfo).hProcess, &exitCode) && exitCode == STILL_ACTIVE)
      {
        ...
        {
          NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHookerProxy : Error close");
        }
...
void MameHookerProxy::Gunshot(int gunIndex)
{
  ...
  if (useSindenRecoil)
  {
    if (gunIndex == 0)
    {
      if (!pipeConnectedSindenGunA)
      {
        ...
        hPipeSindenGunA = CreateFileA("\\\\.\\pipe\\RecoilSindenGunA", GENERIC_WRITE, 0, NULL,
  ...
  NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHooker : Gunshot {}", gunIndex);
...
{
  NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHooker : Start2 game {}", id);
  active = true;
  ...
  std::string arguments =
      "gamename=\"" + id +
      "\" "
      "outputs=\"GunRecoil_P1,GunRecoil_P2,GunRecoil_P3,GunRecoil_P4,TriggerPress_P1,TriggerPress_P2,TriggerPress_P3,TriggerPress_P4,Rumble_P1,Rumble_P2,Rumble_P3,Rumble_P4\"";
  ...
  if (processInfo != nullptr)
  {
     ...
     std::thread([=]() {
      ...
      while (active && compteur < 50)
      {
        ...
        if (hPipe == INVALID_HANDLE_VALUE)
        {
          NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHooker : Not Connected");
          pipeConnected = false;
          
        }
        else
        {
          NOTICE_LOG_FMT(ACHIEVEMENTS, "MameHooker : Connected");
          pipeConnected = true;
          break;
        }
      }
